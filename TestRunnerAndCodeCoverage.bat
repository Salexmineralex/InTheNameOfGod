...
set testRunnerCommand="%ue4Location%\Engine\Binaries\Win64\UnrealEditor-Cmd.exe" "%workspace%\%projectFilename%" -run=Automation -Device=Null -nullrhi -unattended -testexit="Automation Test Queue Empty" -reportoutputpath="%workspace%\%testReportFolder%" -log -logfilename=%testsLogName% -multiprocess -WaitForExit
